...
s=s.replace("facadeData:'', orderStatusBusy:false };", "facadeData:'', orderStatusBusy:false, historyOrderOpenId:'' };", 1)
...
.stock-inline{display:flex;align-items:center;gap:5px;width:fit-content;max-width:100%;margin-top:2px;border-radius:9px;padding:5px 7px;background:#e9f8f1;color:#087653;border:1px solid #bfe8d8;font-size:.59rem;font-weight:950;line-height:1.2}
...
s=s.replace("visible.map(p=>{const stock=stockInfo(p);return `", "visible.map(p=>{const stock=stockInfo(p);const estadoDirecto=stock.className==='is-out'?'AGOTADO · TEMPORALMENTE SIN EXISTENCIAS':stock.className==='is-low'?stock.label:'DISPONIBLE';return `", 1)
s=s.replace("<h2 class=\"product-name\">${esc(p.nombre||'Producto')}</h2>${isAntibiotic(p)?", "<h2 class=\"product-name\">${esc(p.nombre||'Producto')}</h2><div class=\"stock-inline ${stock.className}\">${esc(estadoDirecto)}</div>${isAntibiotic(p)?", 1)
